def setup():
    global cube, cube2, cube3, Obj_model, msx_model
    size(500, 500, P3D)
    colorMode(HSB)
    cube = Cube(0, 0, 0, 1, 60) # Creating the cube
    cube2 = Cube(0, 0, 0, 0.5, 100)
    cube3 = Cube (-200, -150, 0, 2, 200)
    Obj_model = Model("RobotOBJ.obj", 0, 0, 0, 75, 150)
    msx_model = Model("msx.obj", 0, 0, 0, 50, 200)
    
def draw():
    global cube, cube2, cube3, Obj_model, msx_model #Always call variables
    colorMode(RGB)
    blendMode(SUBTRACT)
    fill(255, 1)
